...
def plot_losses(train_losses, val_losses):
    ...
    # epochs_train = np.arange(1, epochs + 1, 1)
    epochs_val = np.arange(1, epochs + 1, 10)

    plt.figure()
    sns.set_theme(style="darkgrid")
    # plt.plot(epochs_train, train_losses[:, 0])
    plt.plot(epochs_val, val_losses[:, 0])
    # plt.plot(epochs_train, train_losses[:, 1])
    plt.plot(epochs_val, val_losses[:, 1])
    plt.xlabel(r"Epoch")
    plt.ylabel(r"Loss")
    ...
    for epoch in range(epochs):
        try:
            ...
            if epoch % 10 == 0:
                ...
                val_losses[epoch // 10] = val_loss
                print(f"Epoch: {epoch:3d}, Train Loss: {train_loss_mean:.4f}, Val Loss: {val_loss.item():.4f}")
...
def main():
    ...
    key, subkey = jax.random.split(key)
    n_train = 10
    x0_train = jax.random.uniform(key, shape=(n_train, 2), minval=-x_width, maxval=x_width)
    x_train = generate_data(function, args, t, x0_train)
    ...
    key, subkey = jax.random.split(key)
    n_val = 100
    x0_val = jax.random.uniform(key, shape=(n_val, 2), minval=-x_width, maxval=x_width)
    x_val = generate_data(function, args, t, x0_val)
